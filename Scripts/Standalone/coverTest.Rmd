...
There are only two conditions where the cover stories make a difference at < 0.01: 2_d7 and 3_d7. (ie., both disjunctive A=1, B=1, E=1, which is the overdetermined 'most important' case). Trialtype d2 (A=0, B=1, E=0) also not robust. 
...
There is still a slight difference where for this important trialtype (D111) the cover stories give rise to different participant behaviours. Especially odd that the cafe one (here called 'cook' for legacy reasons - that didn't affect what participants saw) and the job one are dissociated.

If this was a serious difference, the next thing would be to compare with model predictions. It may also have reporting implications. But - on balance and discussion it can be ascribed to noise - it is not unheard of to have different proportions and we should not expect strictly uniform across the board.