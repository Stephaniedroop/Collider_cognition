...
predP <- glmer(P ~ 1 + (1|trial_id) + (1|subject_id), data = dfab, family = binomial(link='logit')) # Also tried with + (1|subject_id) but no difference in fit, has convergence error and looks overparametrised?
summary(predP)
...
predP2 <- glmer(P ~ structure + (1|trial_id) + (1|subject_id), data = dfab, family = binomial(link='logit')) # Also tried with + (1|subject_id) but no difference in fit, has convergence error and looks overparametrised?
summary(predP2)
...
dab <- dab |> 
  select(subject_id, trial_id, P) 


predPa <- glmer(P ~ 1 + (1|subject_id) + (1|trial_id), data = dab, family = binomial(link='logit')) # Also tried with + (1|subject_id) but no difference in fit, has convergence error and looks overparametrised?
summary(predPa)

coef <- fixef(predPa) # 0.107

est <- exp(coef) # 1.11 exp converts logodds to odds. 
...
lower_or <- exp(lower_logodds) .758
upper_or <- exp(upper_logodds) 1.63
