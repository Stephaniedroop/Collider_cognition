...
```{r, echo=FALSE}
rm(list=ls())
df <- read.csv(params$data_file, stringsAsFactors = T) # 288 obs of 27 vars 
...
### For Neil presentation: just participants
...
**HOWEVER!!** beware: all these plots are made from just the aggregate normalised probabilities. They are not made from *sampled explanations* sampled from the model matched to each participant and condition observation. They are much less sophisticated. That further sampling has been done but the plots have not been made yet. TO DO Weds 10 Sept.



Now just observed v unobserved for everything together, noKind model. It needs chunks to calculate separate long and summary dfs for the new se values - they are different because different numbers of observations in the means.
