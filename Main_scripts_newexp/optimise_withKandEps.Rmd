...
Takes data created in `modelCombLesions.Rmd`, ie. `modelAndDataunfitpn.csv`. Outputs `fit16mpn.csv`, which then goes for some preprocessing in `processForPLot.Rmd` 
and that is then used by the next scripts in this series:  `reportingFigs16m.Rmd` and `samplePredictions.Rmd`.
...
```{r}
write.csv(fitforplot, '../model_data/fit16mpn.csv')  
```