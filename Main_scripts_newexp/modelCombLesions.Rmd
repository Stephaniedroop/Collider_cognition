...
```{r}
#setwd("../../Main_scripts_newexp") # do this if you changed it for the Data processing
```
...
There is more that can be done here to bring in all the vars that later get added in optimise script.




```{r}
write.csv(modelAndData, '../model_data/modelAndDataUnfitpn.csv')  
```
