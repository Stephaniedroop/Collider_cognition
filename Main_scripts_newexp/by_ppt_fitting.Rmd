...
df2.map<-data.frame(condition=c('c1','c2','c3','c4','c5','d1','d2','d3','d4','d5','d6','d7'),
...
for (i in 1:nrow(df2))
{
  df2$A[i]<-df2.map$A[df2.map$condition==df2$trialtype[i]]
  df2$B[i]<-df2.map$B[df2.map$condition==df2$trialtype[i]]
  df2$E[i]<-df2.map$E[df2.map$condition==df2$trialtype[i]]
}

df2 <- df2 %>%
  mutate(include = !( (node3=='B=0' & B==1) | (node3=='B=1' & B==0) | (node3=='A=0' & A==1) | (node3=='A=1' & A==0)))

# Now select columns you need
